...
Even though Java and Eiffel are both object oriented languages, there are differences between these two languages, syntax left aside. Eiffel offers a wider set of language constructs, many times trying to solve problems inherently object oriented, whereas Java reused solutions from its ancestors, mainly C++. One example for this difference are be the Eiffel basic types, which can be treated as objects with methods and attributes versus the Java basic types, which are no objects. This section focuses on the differences between Java and Eiffel that influenced our implementation of selective capture and replay.
...
Emmanuel Stapf advised us to make the modifications in the class \texttt{AST\_FEATURE\_CHECKER\_GENERATOR}, which is used at the stage where the compiler checks the types of the program. Code instrumentation is inserted into the routines before type checking (\texttt{process\_routine\_as}). Our code generates additional AST nodes and replaces the original AST of the routine body. At the moment only regular routines are instrumented, but it should be possible to support once routines, too, without much additional work. 

A simple mechanism to exclude classes and routines from instrumentation was implemented, because some classes like \texttt{PROGRAM\_FLOW\_SINK} need a special treatment to avoid management code invoke itself through instrumentation code. There are also some features that need to be instrumented in a special way, for example \texttt{SPECIAL.note_direct_manipulation} that needs to record the content of the special during capture phase and restore that content during replay phase. At the moment, two arrays are initialized with the names of exceptional classes and features, but it is clear that at later stage this needs to be managable through a configuration file, to avoid recompiling Eiffel Studio, only because these exceptions changed.

%TODO ausschaltbare instrumentierung

\section{Implementation}
In this chapter, we will explain the implementation of the management code. For a quick overview over our implementation, we will first outline the architecture of the solution:
...
\texttt{RECORDER} uses \texttt{CAPTURE\_SERIALIZER} to write events to a log file.
\begin{figure}[ht]
  \centering
