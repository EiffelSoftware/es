...
The parser is divided into the class \class{EVENT\_PARSER} that reads from the log and \class{EVENT\hspace{0pt}\_PARSER\_HANDLER} to handle events that were generated by the parser (\figref{fig:implementation_parser}). \class{\{EVENT\_PARSER\}}\hspace{0pt}.\feature{parse\_event} parses the next event from the log and then calls the appropriate handler from \class{EVENT\hspace{0pt}\_PARSER\_HANDLER}, for example \feature{handle\_incall\_event}.  
